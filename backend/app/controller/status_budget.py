...
def insert_status_budget(status_bud: BaseStatusBudget, db: Session) -> BaseModelStatusBudget:
...
def select_status_budget_by_budget_id(budget_id: id, db: Session) -> BaseModelStatusBudgets:
...
def update_status_budget(
    ...
    db.query(StatusBudget).filter(StatusBudget.id == id).update(row, synchronize_session=False)
    db.commit()
    db.refresh(status)
    return status
